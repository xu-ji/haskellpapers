...
bdd2,bdd3,bdd4,bdd5,bdd6,bdd7,bdd8 :: BDD
bdd2 = [(1,(4,-1,-2))]
bdd3 = [(1,(1,-1,-2))]
...
lookUp :: Eq a => a -> [ ( a, b ) ] -> b
lookUp a pairs = fromJust(lookup a pairs)
...
--checkAndSwitch2 :: Id -> Id
--checkAndSwitch num = lookUp num numTable
...
--  [i1', i2', i3'] = map checkAndSwitch [i1, i2, i3]
...
--    | i1 == n = if ((lookUp id e) == True) then 
--                     (if (i2 < 0) then i2 
--                     else
--                     (checkSat' origBs e i2 (origBs\\b))
--                else
--                     (if (i3 < 0) then i3  
--                     else 
--                     (checkSat' origBs e i3 (origBs\\b)))
...
-- we do not need either exp or inds during most of the execution, only for thebase case.
...
buildBDD' exp currId pos bools inds
  ...
  | otherwise = [(currId, (inds!!pos, currId*2, currId*2+1))] ++
                (buildBDD' exp (currId*2) (pos+1) (False:bools) inds) ++
                (buildBDD' exp (currId*2+1) (pos+1) (True:bools) inds)
