...
accepts ids (p:pdef) = accepts' ids p (p:pdef)
  ...
  accepts' (i:ids) (name, (Prefix id process)) pdefs
    ...
    | otherwise = False
  accepts' (ids) (name, (Choice (pros))) pdefs = or (map (\ pdef -> accepts' (ids) pdef pdefs) (makeDefs name pros))
...
composeTransitions ((s, t), a) ((s', t'), a') alpha1 alpha2 map
  | a == a' = [(((lookUp (s, s') map), (lookUp (t, t') map)), a)]
  | a `elem` alpha2 && a' `elem` alpha1 = []
  | a' `elem` alpha1 = [(((lookUp (s, s') map), (lookUp (t, s') map)), a)]
  | a `elem` alpha2 = [(((lookUp (s, s') map), (lookUp (s, t') map)), a')]
  | otherwise = [(((lookUp (s, s') map), (lookUp (t, s') map)), a), (((lookUp (s, s') map), (lookUp (s, t') map)), a')]
